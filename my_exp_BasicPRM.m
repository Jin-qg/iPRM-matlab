...
for map_i = 1:size(map_names,2)
    ...
    for node_group_i = 1:nodes_group_num
       ...
       %% iter creating repeatedly same amount of nodes
        for node_repeat_i = 1:nodes_repeat_num
            ...
            for cd_i = 1:size(connectDis,2)
                ...
                cell = myEXPFuncBasicPRM(map_name,nodeNum,nodeRepeat,startLocation,endLocation,connectDis(cd_i),safeDistance,display,save);
                data = [data;cell];
            end
       ...
       %% write xls
        con_dis_num = size(connectDis,2);
        output_path = strcat('./result/basic/',map_name,'_bas.xlsx');
        ...
        fprintf('%s : exp basic PRM finished.\n', map_name);
