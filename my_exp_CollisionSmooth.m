...
[map_path_success] = xlsread('./result/path_success.xlsx');
...
for map_i = 1:size(map_names,2)
    ...
    for node_group_i = 1:nodes_group_num
       ...
       %% iter creating repeatedly same amount of nodes
        for node_repeat_i = 1:nodes_repeat_num
            ...
            for cd_i = 1:size(connectDis,2)
                ...
                index_temp = (node_group_i-1)*nodes_repeat_num*con_dis_num...
                     + (node_repeat_i-1)*con_dis_num + cd_i;
                if map_path_success(index_temp,map_i) == 0
                    fprintf('%s-%d-%d-%s: skipped.\n',map_name,nodeNum,nodeRepeat,num2str(connectDis(cd_i)));
                    cell = {nodeNum,nodeRepeat,connectDis(cd_i), 0, Inf,Inf,Inf,Inf,Inf, 0, 0,0,0, 0};
                    data = [data;cell];
                    continue;
                end
                fprintf('%s-%d-%d-%s: ',map_name,nodeNum,nodeRepeat,num2str(connectDis(cd_i)));
                cell = myEXPFuncCollisionSmooth(map_name,nodeNum,nodeRepeat,startLocation,endLocation,connectDis(cd_i),safeDistance,display,save);
                data = [data;cell];
            end
       ...
       %% write xls
        con_dis_num = size(connectDis,2);
        output_path = strcat('./result/collision_smooth/',map_name,'_cs.xlsx');
        line_start = 3 + (node_group_i-1)*nodes_repeat_num*con_dis_num;
        line_end = line_start + nodes_repeat_num*con_dis_num - 1;
        xlrange = sprintf('B%d:O%d',line_start,line_end);
        xlswrite(output_path, data, xlrange);
        
        fprintf('%s : exp collision and smooth finished.\n', map_name);
